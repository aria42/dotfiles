...
;; Copyright 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013 Kevin Ryde
...
;;            - org-mode $...$ is latex
;; Version 13 - some byte compiler quietening
;; Version 14 - fix `tex-math-preview-adaptview' image availability condition
;; Version 15 - fix latex \usepackage inclusion
...
(eval-when-compile
  (require 'cl)) ;; emacs20 `dolist',`ignore-errors'

;; from org.el, quieten the byte compiler
...
This is an internal part of tex-math-preview.el.

Packages can have an optional argument part,

    \\=\\usepackage{foo}
    \\=\\usepackage[arg arg arg]{foo}

and that part can extend across multiple lines

    \\=\\usepackage[arg,
                    arg,
                    arg]{foo}

Not sure about the escaping rules within the arg string.  In the
current code a \\=\\] is taken to be part of the arg and not its
] end.  There shouldn't be any such things in the package name
part."
  (save-excursion
    (goto-char (point-min))
    (let ((ret ""))
...
;; locally bound, though might be able to avoid that ...
(defvar tex-math-preview-dir)
...
STR should not have $ or $$ delimiters."
  ...
  (let* ((orig-buffer  (current-buffer))
         (tex-math-preview-dir (make-temp-file "tex-math-preview-" t))
         (dot-tex      (expand-file-name "foo.tex" tex-math-preview-dir))
         (dot-dvi      (expand-file-name "foo.dvi" tex-math-preview-dir))
         (dot-log      (expand-file-name "foo.log" tex-math-preview-dir))
         (dot-aux      (expand-file-name "foo.aux" tex-math-preview-dir))
         (dot-out      (expand-file-name "foo.out" tex-math-preview-dir))
    ...
    (with-temp-file dot-tex
      (if (equal "tex" tex-cmd)
        ...
        (insert (with-current-buffer orig-buffer
                  (tex-math-preview-usepackages)))
    ...
    (unwind-protect
        ...
        (let ((max-mini-window-height 1)  ;; force shell-command to buffer
          ...
          (if (not (eq 0 (shell-command
                          (concat tex-cmd
                                  " -output-directory " (shell-quote-argument tex-math-preview-dir)
                                  " " (shell-quote-argument dot-tex)
      ...
      ;; cleanup temp files
      (dolist (filename (list dot-tex dot-dvi dot-log dot-aux dot-out))
        (ignore-errors (delete-file filename)))
      (delete-directory tex-math-preview-dir))))
...
`tex-math-preview-png-image' is used if possible, or if not then
`tex-math-preview-dvi-view' for the `tex-mode' previewer given by
...
capabilities and available viewer program(s)."
  ...
  (if (and (fboundp 'image-type-available-p) ;; not in emacs20
           (image-type-available-p 'png)
           (display-images-p)
           (eq 0 (call-process "dvipng"
                               nil ;; stdin /dev/null
...
find a \"quiet\" mode or use a wrapper script to grep that out."
  ...
  (let* ((template (eval tex-dvi-view-command))
    ...
    (if (string-equal command template)
        (setq command (concat command " " (shell-quote-argument filename))))
...
normally per `auto-coding-alist'.)"
  ...
  (unless (and (fboundp 'image-type-available-p) ;; not in emacs20
               (image-type-available-p 'png)
               (display-images-p))
    (error "Cannot display PNG in this Emacs"))

  (let ((dot-png (expand-file-name "foo.png" tex-math-preview-dir))
        (inhibit-read-only t))
    (when (eq 0 (shell-command (concat "dvipng -T tight -x 1728"
                                       "  -o" (shell-quote-argument dot-png)
...
(defun tex-math-preview-unload-function ()
  "Undo tex-math-preview.el setups.
This is called by `unload-feature'."
  (put 'tex-math 'bounds-of-thing-at-point nil)
  nil) ;; and do normal unload-feature actions too

;; LocalWords: tex LaTeX Texinfo amsmath DBTexMath usepackage dvi xdvi dvisvga tmview png dvipng ghostscript zgv dvips http www emacswiki LaTeXMathPreview el tty ified startup
